...
const generateBulkCertificates = asyncHandler(async (req, res) => {
  ...
  if (!finalSignatureUrl) {
    ...
    const signatureImage = await uploadOnCloudinary(signatureLocalPath, 'Devspace/certificate');
    if (!signatureImage) {
  ...
  for (const student of students) {
    ...
    try {
      ...
      const qrUpload = await uploadOnCloudinary(dataUri, 'Devspace/certificate');
      if (qrUpload) {
...
const verifyCertificate = asyncHandler(async (req, res) => {
  ...
  const { data: certificate, error } = await supabase.from('certificates').select('*').eq('certificateId', certificateId).single();
...
const getLatestSignature = asyncHandler(async (req, res) => {
  const { data: latestCertificate, error } = await supabase
    .from('certificates')
  ...
  if (error || !latestCertificate || !latestCertificate.signatureImage) {
