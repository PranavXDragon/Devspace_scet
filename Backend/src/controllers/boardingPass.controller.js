...
const generateBulkBoardingPasses = asyncHandler(async (req, res) => {
  ...
  for (const student of students) {
    ...
    try {
      ...
      const qrUpload = await uploadOnCloudinary(dataUri, 'Devspace/pass');
      if (qrUpload) {
    ...
    const { data: pass, error: insertError } = await supabase.from('boarding_passes').insert([{
    ...
    if (insertError) {
      console.error("Error inserting boarding pass:", insertError);
...
const verifyBoardingPass = asyncHandler(async (req, res) => {
  ...
  const { data: boardingPass, error } = await supabase.from('boarding_passes').select('*').eq('boardingPassId', boardingPassId).single();
