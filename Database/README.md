...
- **`CodeXDB.png`**: The Entity-Relationship (ER) diagram representing the database architecture of the CodeX platform. This visually outlines the core tables (such as Users, Events, Registrations, Teams) and the references between them.

## Database Technology

The CodeX project utilizes **Supabase (PostgreSQL)** as its primary data store, managed via the Supabase JS client in the Node.js backend. 
