        ...
        <div className="relative h-[550px] w-full flex justify-center items-center mt-12 xl:mt-0">
          ...
          <div className="absolute inset-0 flex justify-center items-center perspective-[1200px]">
             {eventsHero.cards.map((card) => {
               ...
               let zIndex = "";
               if (card.id === 1) {
                 posClasses = "rotate-0 scale-100 -translate-y-4 shadow-[0_20px_50px_rgba(0,0,0,0.5)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.8)]";
                 zIndex = "z-30";
               } else if (card.id === 2) {
                 posClasses = "-translate-x-[55%] translate-y-8 -rotate-[12deg] scale-[0.85] opacity-80 shadow-[0_15px_30px_rgba(0,0,0,0.4)]";
                 zIndex = "z-10";
               } else if (card.id === 3) {
                 posClasses = "translate-x-[55%] translate-y-8 rotate-[12deg] scale-[0.85] opacity-80 shadow-[0_15px_30px_rgba(0,0,0,0.4)]";
                 zIndex = "z-20";
               ...
               return (
                 <div key={card.id}className={`absolute w-[260px] h-[380px] lg:w-[300px] lg:h-[440px] transition-all duration-500 rounded-[1.5rem] overflow-hidden border-[2px] border-border-soft hover:border-accent/80 hover:shadow-[0_0_30px_rgba(46,197,212,0.3)] bg-card/40 backdrop-blur-md group ${posClasses} ${zIndex}`}style={{ 
                      clipPath: card.id === 1 
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black via-black/40 to-transparent z-20 opacity-80 group-hover:opacity-60 transition-opacity duration-300"></div>
                    <div className="w-full h-full relative z-10 flex items-center justify-center bg-card/60 border border-border-soft/50">
                      <span className="text-text-muted/20 font-sans font-bold text-2xl uppercase tracking-[0.3em] rotate-[-15deg]">IMAGE</span>
                    </div>
